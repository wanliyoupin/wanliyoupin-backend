...
type PackageItem = {
  ...
  _companyId?: number;
  package_product_skus?: { id: number; quantity?: number; sort_order?: number; product_sku?: { id: number; name: string; price: number; product?: { name: string } } }[];
...
function CategoryTreeFilter({
  ...
  return (
    <ul className={depth ? "ml-3 border-l border-slate-200 pl-2" : ""}>
      ...
      {nodes.map((node) => {
        const count = node.packages_aggregate?.aggregate?.count ?? 0;
        ...
        return (
          <li key={node.id} className="mb-1">
            <button
              ...
              title={`${node.name} · ${mode.text}。共 ${count} 个套餐`}
            >
              <div className="flex items-start justify-between gap-1">
                ...
                <span className="text-slate-500 text-xs flex-shrink-0 tabular-nums">({count})</span>
              </div>
            </button>
...
export default function PackagesPage() {
  ...
  const load = async (pageNum: number) => {
    ...
    if (shelveFilter === "shelved") params.set("is_shelved", "true");
    if (shelveFilter === "unshelved") params.set("is_shelved", "false");
    if (categoryId != null) params.set("categoryId", String(categoryId));
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/company/packages?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (res.ok) {
        ...
        setHiddenPackageIds(data.hiddenPackageIds ?? []);
  ...
  const handleDelete = async (pkg: PackageItem) => {
    if (!token || !confirm(`确定要删除套餐「${pkg.name}」吗？`)) return;
    try {
      const res = await fetch(`/api/admin/company/packages/${pkg.id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const toggleShelve = async (pkg: PackageItem) => {
    if (!token || !isAdminForSelectedCompany) return;
    const next = !pkg.is_shelved;
    try {
      const res = await fetch(`/api/admin/company/packages/${pkg.id}`, {
        method: "PATCH",
        headers: {
      ...
      if (res.ok) {
        setPackages((prev) =>
          prev.map((p) => (p.id === pkg.id ? { ...p, is_shelved: next } : p))
        );
      } else {
        const err = await res.json();
  ...
  const updateCompanyHidden = async (ids: number[]) => {
    ...
    const res = await fetch(`/api/admin/company/${effectiveCurrentCompanyId}`, {
      ...
      body: JSON.stringify({ hidden_package_ids: ids }),
    });
    if (res.ok) {
      setHiddenPackageIds(ids);
  ...
  const handleHide = (packageId: number) => {
    if (hiddenPackageIds.includes(packageId)) return;
    updateCompanyHidden([...hiddenPackageIds, packageId]);
  };

  const handleUnhide = (packageId: number) => {
    updateCompanyHidden(hiddenPackageIds.filter((id) => id !== packageId));
  };
  ...
  const handleExportExcel = async () => {
    ...
    try {
      while (true) {
        ...
        const list = (data.packages ?? []) as PackageItem[];
        if (offset === 0) hiddenIds = data.hiddenPackageIds ?? [];
        all.push(...list);
      ...
      downloadPackagesExcel(filtered);
    } catch {
      alert("导出失败");
    } finally {
      setExporting(false);
  ...
  const isFromSystem = (p: PackageItem) =>
    systemCompanyId != null && (p.company_companies === systemCompanyId || p._companyId === systemCompanyId);
  const isHidden = (p: PackageItem) => hiddenPackageIds.includes(p.id);
  ...
  if (!effectiveCurrentCompanyId) {
    return (
      <div>
        <h1 className="text-xl font-semibold text-slate-800 mb-2">套餐管理</h1>
    ...
    <div className="flex gap-4 flex-1 min-h-0">
      ...
      <div className="flex-1 min-w-0 min-h-0 flex flex-col">
        ...
        <div className="flex-shrink-0 flex items-center justify-between mb-4 flex-wrap gap-2">
          <h1 className="text-xl font-semibold text-slate-800">套餐管理</h1>
          <div className="flex items-center gap-2">
            ...
            {canEdit && (
              <Link
                href="/dashboard/company/packages/new"
        ...
        <div className="flex-shrink-0 mb-4 flex flex-col gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-sm font-medium text-slate-600">上架状态：</span>
            {(["all", "unshelved", "shelved"] as const).map((key) => (
              <button
                key={key}
                type="button"
              ...
              >
                {key === "all" ? "全部" : key === "shelved" ? "已下架" : "已上架"}
              </button>
            ))}
          </div>
        </div>
        ...
        {loading && packages.length === 0 ? (
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-8 text-center text-slate-600">
            ...
            {canEdit && (
              <div className="mt-2">
                <Link href="/dashboard/company/packages/new" className="text-indigo-600 hover:underline">
                  添加套餐
                </Link>
              </div>
            )}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm text-slate-500">
                    {pkg.package_product_skus?.length ?? 0} 个商品
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <Link
                    href={`/dashboard/company/packages/${pkg.id}/preview`}
                  ...
                  {effectiveCurrentCompanyId && (
                    <button
                      type="button"
                      disabled={wxacodeGenerating === pkg.id}
                      onClick={() => handleGenerateWxacode("package", pkg.id, effectiveCurrentCompanyId)}
                      className="text-sm text-indigo-600 hover:underline disabled:opacity-50"
                    >
                      {wxacodeGenerating === pkg.id ? "生成中…" : "小程序码"}
                    </button>
                  )}
                  {canEdit && (
                    isFromSystem(pkg) && effectiveCurrentCompanyId !== systemCompanyId ? (
                      isHidden(pkg) ? (
                        <button
                          type="button"
                          onClick={() => handleUnhide(pkg.id)}
                      ...
                      <>
                        <button
                          type="button"
                          onClick={() => toggleShelve(pkg)}
                          className="text-sm text-slate-600 hover:underline"
                        >
                          {pkg.is_shelved ? "上架" : "下架"}
                        </button>
                        <Link
                          href={`/dashboard/company/packages/${pkg.id}/edit`}
                          className="text-sm text-indigo-600 hover:underline"
                        >
                          编辑
                        </Link>
