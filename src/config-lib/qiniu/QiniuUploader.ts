...
export class QiniuUploader {
  ...
  constructor(config: QiniuConfig = qiniuConfig) {
    ...
    this.putPolicy = {
      scope: config.bucket,
      expires: 86400, // 24 小时，避免长时间运行后 token 过期
    };
  ...
  async uploadFile(
    ...
    const uploadToken = new qiniu.rs.PutPolicy(this.putPolicy).uploadToken(
      this.mac
    );
      ...
      this.formUploader.put(
        ...
        (err, body, info) => {
          ...
          if (info.statusCode === 200) {
            ...
            reject(new Error(`七牛云上传失败: ${msg || `HTTP ${info.statusCode}`}`));
          }
        }
      );
    });
